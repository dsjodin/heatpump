"""
Thermia Data Query & Calculations - KORRIGERAD VERSION
Handles all InfluxDB queries and advanced calculations

KORRIGERINGAR:
1. Varmvattenberäkning använder nu verklig effekt under cykler
2. Flexibel aggregering baserat på tidsperiod
3. Bättre tidshantering i alla beräkningar
4. Suppression av InfluxDB pivot-varningar (queries fungerar korrekt)
"""
...
# Thermia larmkoder och beskrivningar
ALARM_CODES = {
    0: "Inget larm",
...
class ThermiaDataQuery:
            ...
            for start_time in cycles_start['_time']:
                ...
                if not end_times.empty:
                    ...
                    if duration_minutes < MIN_CYCLE_DURATION_MINUTES:
                        filtered_count += 1
                        logger.info(f"  ❌ FILTRERAD kort cykel kl {start_time.strftime('%H:%M:%S')}: {duration_minutes:.1f} min (< {MIN_CYCLE_DURATION_MINUTES} min)")
                        continue
                    ...
                    if not cycle_power.empty:
                        ...
                        logger.info(f"  ✅ GILTIG cykel kl {start_time.strftime('%H:%M:%S')}: {duration_minutes:.1f} min, {total_cycle_energy:.2f} kWh")
    ...
    def get_alarm_status(self) -> Dict[str, Any]:
        """Get current alarm status with description"""
        try:
            ...
            alarm_description = ALARM_CODES.get(alarm_code, f"Okänd larmkod: {alarm_code}")
            ...
            for metric in metrics:
                ...
                for idx, row in result.iterrows():
                    ...
                    # Larm
                        ...
                        if current > 0 and previous == 0:
                            alarm_desc = ALARM_CODES.get(int(current), f"Kod {int(current)}")
                            events.append({
                                'time': timestamp,
